...
void SampleApp_Init( uint8 task_id )
{
  ...
  P0SEL &= ~0xF2;               //设置P0.6口和P0.7为普通IO
  P0DIR |= 0xF2;                //设置P0.6口和P0.7口为输出
  
//自行定义IO口模式
  
  P1SEL &= ~0xD6;               //设置P1.0口为普通IO，设置P1.7口为普通IO，设置P1.6口为普通IO口
  P1DIR |= 0xD6;
...
//自行定义的事件与处理函数绑定
//  osal_set_event( SampleApp_TaskID, SAMPLEAPP_ZHENG_EVT );
  ...
  if ( events & SAMPLEAPP_ZHENG_EVT )
  {
    if( SportState == 1 )
    {
      if( SampleAppState == 0  )
      {
        ...
        SampleAppState = 1;
        osal_start_timerEx( SampleApp_TaskID, SAMPLEAPP_ZHENG_EVT, 5 );
      }
      else if( SampleAppState == 1 )
      {
        ...
        Driver_4 = 0;
        SampleAppState = 0;
        osal_start_timerEx( SampleApp_TaskID, SAMPLEAPP_ZHENG_EVT, 15 );
      }
    }
    ...
    return( events ^ SAMPLEAPP_ZHENG_EVT );
  }
//自行定义的事件处理函数
  
  if( events & SAMPLEAPP_COLOR_EVT ) //此时检测到黑带
  {
    if( SportState == 1 )
    {
