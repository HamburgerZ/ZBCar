...
void SampleApp_DetectBlack(void)
{
      ...
      else if( Color_1 == 0 && Color_2 == 0 ) //此时两灯都亮起，没有检测黑带。
      {
        SampleAppDirection = 0;
        osal_start_timerEx( SampleApp_TaskID, SAMPLEAPP_COLOR_EVT, 50 );
      }    
      else if( Color_1 == 1 && Color_2 == 1 ) //此时两灯都熄灭，检测到黑带。
      {
...
static void SerialApp_CallBack( uint8 port, uint8 event )
{
     ...
     HalUARTRead( 0, Uartbuf, 10 );
     if( osal_memcmp( Uartbuf, "HamburgerZ", 10 ) )
     {
     HalUARTWrite( 0, Uartbuf, 10 );
     }
}
*/
